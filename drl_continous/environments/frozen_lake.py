...
class ContinuousFrozenLake(Env):
    ...
    def __init__(self, map_name: str = "6x6", reward_type: RewardType = RewardType.dense, is_slippery: bool = True, is_rendered: bool = True):
        ...
        self.complete_Q = np.load(f"data/q_tables_{map_name}.npz")['q_table_a1']
    ...
    def step(self, action):
        ...
        if self.is_slippery:
            new_x = np.clip(new_x + np.random.normal(0, 0.01), 0.1, self.size - 0.1)
            new_y = np.clip(new_y + np.random.normal(0, 0.01), 0.1, self.size - 0.1)
    ...
    def reset(self) -> Tuple[np.ndarray, dict]:
        ...
        self.num_steps = 0
        self.observation = np.array([0.5, 0.5]) # Start state: bottom left corner
    ...
    def init_pygame(self):
        ...
        self.cracked_hole_img = scale(load(cracked_hole_img_path), (self._cell_size, self._cell_size))
        
        goal_img_path = os.path.join(IMG_DIR, "goal.png")
        self.goal_img = scale(load(goal_img_path), (self._cell_size // 3, self._cell_size // 3))