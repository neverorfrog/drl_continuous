...
class StandardReplayBuffer:
    def __init__(self, env_params, capacity=4096):
        ...
        self.size = 0
        self.buffer = {
            "observation": np.empty([capacity, env_params["obs_dim"]]),
    ...
    def sample(self, batch_size=32):
        ...
        observations = torch.as_tensor(
            self.buffer["observation"][sampled_indices], dtype=torch.float32
        )
        actions = torch.as_tensor(
            self.buffer["action"][sampled_indices], dtype=torch.float32
        )
        rewards = torch.as_tensor(
            self.buffer["reward"][sampled_indices], dtype=torch.float32
        )
        dones = torch.as_tensor(
            self.buffer["done"][sampled_indices], dtype=torch.float32
        )
        new_observations = torch.as_tensor(
            self.buffer["new_observation"][sampled_indices],
            dtype=torch.float32,
        )
