...
from drl_continous.agents.networks import Actor, Critic, SquashedGaussianActor
import os
...
class SAC:
    def __init__(self, name, env: gym.Env, window = 100,
                 polyak = 0.995, pi_lr = 0.0005, q_lr = 0.0005, target_update_freq = 2, value_update_freq = 1, 
                 eps = 1.0, eps_decay = 0.99, batch_size = 32, gamma=0.99, max_episodes=200, reward_threshold=10000000,
                 target_noise = 0.3, noise_clip = 0.3, policy_update_freq = 2, alpha = 0.1):
        ...
        # Networks
        self.actor: SquashedGaussianActor = SquashedGaussianActor(self.env_params).to(device)
    ...
    def select_action(self, observation) -> np.ndarray:
        with torch.no_grad():
            action, _ = self.actor(torch.as_tensor(observation, dtype=torch.float32), with_logprob=False)
        return action
    ...
    def value_learning_step(self, batch): 
        ...
        with torch.no_grad():
            actions, log_pi = self.actor(new_observations) # (batch_size, action_dim)
            log_pi = log_pi.unsqueeze(1) # hotfix
            target_values = torch.min(self.target_critic1(new_observations, actions), self.target_critic2(new_observations, actions))
            targets = rewards + (1 - dones) * self.gamma * (target_values - self.alpha * log_pi)
    ...
    def policy_learning_step(self, batch):
        ...
        estimated_values: torch.Tensor = torch.min(self.critic1(observations, estimated_actions), self.critic2(observations, estimated_actions))
        policy_loss: torch.Tensor = (self.alpha * log_pi - estimated_values).mean() # perform gradient ascent
    ...
    def episode_update(self):
        ...
        print(f'\rEpisode {self.ep} Mean Reward: {meanreward:.2f} Ep_Reward: {self.ep_reward}\t\n')
    ...
    def evaluate(self, env = None, render:bool = True, num_ep = 3):
        ...
        for i in range(1, num_ep+1):
            ...
            while not terminated and not truncated:
                with torch.no_grad():
                    action, _ = self.actor(observation, deterministic=True, with_logprob=False)
